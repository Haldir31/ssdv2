...
while IFS= read -r line || [ -n "${line}" ]; do
  ...
  if [ ${in_env} -eq 1 ] && printf '%s' "${line}" | grep -qE '^[[:space:]]{2,}[A-Za-z_][A-Za-z0-9_]*:'; then
    ...
    v="$(unquote "${v}")"
    v=$(printf '%s' "${v}" | sed "s#__APP_SRC_DIR__#${APP_SRC_DIR}#g;s#__APP_DATA_DIR__#${APP_DATA_DIR}#g;s#__STORAGE_ROOT__#${STORAGE_ROOT}#g;s#__SETTINGS_SOURCE__#${SETTINGS_SOURCE}#g;s#__HOME__#${HOME}#g")
  ...
  case "${line}" in
    *:*)
      ...
      val="$(unquote "${val}")"
      val=$(printf '%s' "${val}" | sed "s#__APP_SRC_DIR__#${APP_SRC_DIR}#g;s#__APP_DATA_DIR__#${APP_DATA_DIR}#g;s#__STORAGE_ROOT__#${STORAGE_ROOT}#g;s#__SETTINGS_SOURCE__#${SETTINGS_SOURCE}#g;s#__HOME__#${HOME}#g")
      case "${key}" in
        kind) kind="${val}" ;;
        repo) repo="${val}" ;;
        ref) ref="${val}" ;;
